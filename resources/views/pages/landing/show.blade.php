    ...
    <div class="kt-container-fixed space-y-5">
        <div class="kt-card px-5">
            ...
            <div class="kt-card-header space-y-5 py-10">
                <!-- Title -->
                <h1 class="text-4xl font-semibold text-mono">
                    {{ $data['article']->title }}
                </h1>
                ...
                <div class="flex flex-col lg:flex-row items-start lg:items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <i class="ki-filled ki-profile-circle text-mono"></i>
                        <span class="text-sm font-medium text-mono">{{ $data['article']->user->name }}</span>
                    </div>

                    <!-- Category -->
                    <div class="flex items-center gap-2">
                        <i class="ki-filled ki-category text-mono"></i>
                        <span class="text-sm font-medium text-mono">{{ $data['article']->category->name }}</span>
                    </div>

                    <!-- Date -->
                    <div class="flex items-center gap-2">
                        <i class="ki-filled ki-calendar text-mono"></i>
                        <span
                            class="text-sm font-medium text-mono">{{ $data['article']->created_at->format('d M Y') }}</span>
                    </div>
                </div>
            ...
            <div class="kt-card-footer flex flex-col items-start gap-5">
                ...
                <div class="w-full space-y-5">
                    <h3 class="text-lg font-semibold text-mono">
                        <i class="ki-filled ki-message-text text-mono mr-2"></i>
                                ...
                                <div class="kt-card kt-card-bordered px-5 py-3">
                                    <div class="kt-card-body">
                                        <div class="flex flex-col items-start gap-2">
                                            <!-- User Avatar -->
                                            <div class="flex items-center gap-2">
                                                <i class="ki-filled ki-profile-circle text-mono text-xl"></i>
                                                <div>
                                                    <span
                                                        class="font-medium text-mono">{{ $comment->user->name }}</span>
                                                    <span class="text-xs text-mono">•</span>
                                                    <span
                                                        class="text-xs text-mono">{{ $comment->created_at->diffForHumans() }}</span>
                                                </div>
                                            </div>
                                            <p class="text-mono">{{ $comment->content }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                        <div class="text-center py-8">
                            <i class="ki-filled ki-message-text text-4xl text-mono mb-2"></i>
                            <p class="text-mono">Belum ada komentar. Jadilah yang pertama berkomentar!</p>
                        </div>
                    @endif
                    ...
                    @else
                        <div class="text-center">
                            <p class="text-mono">Silakan <a href="{{ route('login') }}" class="kt-link">masuk</a> untuk
                                berkomentar.</p>
                        </div>
                    @endauth
